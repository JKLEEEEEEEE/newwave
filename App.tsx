...
// --- Centralized Raw Data (외부 코드에서 가져온 7개 딜 데이터) ---
...
const generateModules = (raw: any): ScoringModule[] => {
  const r = raw || DEAL_RAW_DATA.case1;
  return [
    {
      id: 'm1', title: '1. 상환능력 (Cash Flow)', description: '현금흐름 및 부채상환 커버리지', totalScore: 0,
      details: [
        { category: '현금흐름', item: 'EBITDA 규모(절대금액)', score: 5, value: r._meta ? r._meta.mainMetric : r.dealSize, weight: 15 },
...
const INITIAL_DEALS: DealSummary[] = Object.keys(DEAL_RAW_DATA).map(key => {
  ...
  const modules = generateModules(raw);
  ...
  return {
    ...
    mainMetric: { label: 'Deal Size', value: raw.dealSize },
...
const getAnalysisData = (caseId: string, allDeals: DealSummary[]): VerificationData => {
  ...
  const modules = generateModules(dealRaw);
        ...
        onUploadSuccess={(runId) => {
          ...
          fetch(`http://localhost:3001/api/im/run/${runId}`)
            ...
            .then((data: APIScoringResult) => {
              ...
              const terms = llm?.deal_terms || data.deal_terms;
              ...
              const sponsorName = terms?.SPONSOR?.extracted_value || 'Midas Private Equity';
